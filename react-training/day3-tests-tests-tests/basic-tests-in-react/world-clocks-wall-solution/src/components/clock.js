...
class Clock extends React.Component {
    ...
    toggleEdit() {
        ...
        if (!isEditing) {
            this.props.onLocationUpdate(this.props.location.id, Location(this.state.tempLabel, this.state.tempTimeZone));
        }
        this.setState({
    ...
    render() {
        ...
        return <div className="clock">
            ...
            <div className="clock-label">
                {this.state.isEditMode ? <input type="text" value={this.state.tempLabel} onChange={this.onLabelUpdate} ref={labelInput => this.labelInput = labelInput} /> : 
                this.props.location.label }
            </div>
            {this.state.isEditMode ? null : <button className="remove-button" onClick={this.props.onRemove}>Remove</button>}
