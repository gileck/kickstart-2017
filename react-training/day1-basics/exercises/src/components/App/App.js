...
const Container = ({title, description, exampleProps, children}) => {
  return (<div className={'container'}>
    ...
    <p>{description}</p>
    <h3>Example props</h3>
    <pre><code>{JSON.stringify(exampleProps, null, 4)}</code></pre>
...
const sandboxData = [
  ...
  {
    name: 'TodoItem2',
    description: 'Todo item v2! Enhances <TodoItem> from before so that now it has a small trash (🗑) button, and when clicking on the button, the "onRemove" callback will be called.',
    props: {
      completed: false,
      title: 'Buy todo apps',
    ...
    <div id='app'>
      ...
      {sandboxData.map((exercise, i) => {
        ...
        return (<Container title={`${i + 1}. ${exercise.name}`} description={exercise.description} exampleProps={exercise.props} key={i}>
          {Comp ? <Comp {...exercise.props}>{exercise.children}</Comp> : <p style={{color: 'red'}}>Nothing written yet. Make sure to expose a a component called <strong>{exercise.name}</strong> from <em>Components.js</em></p>}
        </Container>);
      })}
